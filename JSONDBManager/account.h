	...
	//cookieを操作する動作はすべてここに投げる。
	   ...
	   */
	   void init(){
		   ...
		   if (accountCookie.getCookieValue(constants.JSP_SESSION_COOKIE_NAME) == "") {
			   accuntSession.load();
			   accountCookie.setCookie(constants.JSP_SESSION_COOKIE_NAME, accuntSession.getId(), constants.COOKIE_EXPIRATION);
		   }
		   //DBへの接続を開始する
		   connect();
	   }
	   ...
	   */
	   void login(std::string jsonString) {
		   ...
		   //sessionIDを更新する。
		   accountCookie.setCookie(constants.JSP_SESSION_COOKIE_NAME, accountCookie.getCookieValue(constants.JSP_SESSION_COOKIE_NAME), constants.COOKIE_EXPIRATION);
	   ...
	   */
	   bool loginCheck(){
		   ifstream ifs((accountCookie.getCookieValue(constants.JSP_SESSION_COOKIE_NAME)).c_str());	//sessionが保存されているかチェックするためのファイルストリーム

		   bool retBoo = false; //返却値を格納する変数を宣言する

								   //セッション変数のユーザIDを参照し、値が存在するかどうかをチェックする
								   //また、cookieに保存されているsessionと保存しているsessionが同じかチェックする。
	   if (accountCookie.getCookieValue(constants.JSP_SESSION_COOKIE_NAME) != "" && accountCookie.getCookieValue(constants.USER_ID) != "" && ifs.is_open() == true) {
		   //返却値の変数にtrueを格納する
		   retBoo = true;
	   ...
	   void pageCheck(std::string strAuthority){
		   ...
		   int pageAuth = std::stoi(accountCookie.getCookieValue(constants.PAGE_AUTH), nullptr, 16);
			...
			if ((authority & pageAuth) == 0) {
				...
				if (accountCookie.getCookieValue(constants.USER_ID) != "" && accountCookie.getCookieValue("JSESSION") == "") {
					//タイムアウトとして、１をセットする。
					outValue = "1";
				}
